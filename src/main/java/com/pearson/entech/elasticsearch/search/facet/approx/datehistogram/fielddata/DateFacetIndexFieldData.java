...
/**
 */
public class DateFacetIndexFieldData extends AbstractIndexFieldData<AtomicNumericFieldData> implements IndexNumericFieldData<AtomicNumericFieldData> {
    ...
    public static class Builder implements IndexFieldData.Builder {
        ...
        public IndexFieldData build(final Index index, @IndexSettings final Settings indexSettings, final FieldMapper.Names fieldNames,
            ...
            return new DateFacetIndexFieldData(index, indexSettings, fieldNames, type, cache);
        }
    }

    public DateFacetIndexFieldData(final Index index, @IndexSettings final Settings indexSettings, final FieldMapper.Names fieldNames,
            final FieldDataType fieldDataType, final IndexFieldDataCache cache) {
    ...
    public AtomicNumericFieldData loadDirect(final AtomicReaderContext context) throws Exception {
        ...
        try {
            ...
            while((term = iter.next()) != null) {
                final long value = NumericUtils.prefixCodedToLong(term);
                values.add(value);
                if(value > max) {
                    max = value;
                }
            ...
            */
            ...
            if(!build.isMultiValued() && CommonSettings.removeOrdsOnSingleValue(fieldDataType)) {
                ...
                final long singleValuesArraySize = reader.maxDoc() * RamUsage.NUM_BYTES_LONG
                        + (set == null ? 0 : set.getBits().length * RamUsage.NUM_BYTES_LONG + RamUsage.NUM_BYTES_INT);
                final long uniqueValuesArraySize = values.size() * RamUsage.NUM_BYTES_LONG;
                ...
                final long[] sValues = new long[reader.maxDoc()];
